...
void compute_theta_site (population *p, simulation_parameters *sp, statistics *result,
	...
	if (set_mask & (1 << (j * 2))) {
	  ...
	  double nc        = sp->i.assumed_gl_variance * (double) result->ng_size[j];
...
void run_trials (simulation_parameters *sp) {
  ...
  for (unsigned int t = 0; t < sp->g.trials; t++) {
    ...
    if (sp->o.print_sequences)
      print_sequences (parents, sp, false);
    else if (sp->o.simulation_only) {
      print_sequences (parents, sp, true);
      printf ("\n");
    }

    // Now select a subset and print out D values.
    if (! sp->o.simulation_only)
      print_d_values (parents, sp);
  }
...
unsigned int parse_options (simulation_parameters *sp, FILE *f) {
  ...
  while (! feof (f)) {
	...
	(int_directive (f, s, "trials:", &sp->o.trials) ||
	 ...
	 (int_directive (f, s, "assumed-gl-variance:", &sp->i.assumed_gl_variance) &&
